<?php
	...
	class Model_Main extends Model {
		...
		public function add_task($user_id, $add_text) {
			...
			if ($run_add_task) {
				$_SESSION['mess'] = "<p class='text_mess'>Добавлено</p>";
				header("Location:/");
			} else {
				$_SESSION['error'] = "<p class='text_error'>Ошибка!</p>";
			}
		}
		...
		public function remove_all($id) {
			...
			$_SESSION['mess'] = "<p class='text_mess'>Удалено</p>";
			header("Location:/");
		}
		...
		public function status($id) {
			...
			if ($out_task['status'] == 1) {
				...
				$_SESSION['mess'] = "<p class='text_error'>Ошибка!</p>";
				header("Location:/");
			}
