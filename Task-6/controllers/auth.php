			...
			if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
				...
				if ($num_user == 1) {
					if (password_verify($password, $out_user['password'])) {
						...
						$_SESSION['error'] = "<p class='text_error'>Неверная почта или пароль!</p>";
					...
					if ($run_add_user == true) {
						...
						$_SESSION['mess'] = "<p class='text_mess'>УЧЕТНАЯ ЗАПИСЬ УСПЕШНО СОЗДАНА!</p>";
						header("Location: ../index.php");

					} else {
						$_SESSION['error'] = "<p class='text_error'>Ошибка регистрации</p>";
						header("Location: ../authorization.php");
					}
				} else {
					$_SESSION['error'] = "<p class='text_error'>Не удалось авторизоваться</p>";
					header("Location: ../authorization.php");
				}
			} else {
				$_SESSION['error'] = "<p class='text_error'>Адрес электронной почты введен неверно!</p>";
				header("Location: ../authorization.php");
			}
		} else {
			$_SESSION['error'] = "<p class='text_error'>Вы не ввели данные!</p>";
			header("Location: ../authorization.php");
		}
	}
